...
% N         = total number of modes for normal velocity
    ...
    zi=sqrt(-1);
    %...input data
    iflow  = input('Jets (1) or Couette flow (2) ');
    ...
    if (iflow == 1)
      [A,B] = JetsMatrix(N,alpha,beta,Re);
    else
    ...
    %...graphics
    figure(1)
    fill(imag(numran),real(numran),[0.7 0.7 0.7])
    hold on; 
    plot(imag(numran),real(numran),'b')    
    plot([0 0],[-300 15],'r','LineWidth',2)
    plot(imag(e),real(e),'k.','MarkerSize',18);
    hold off
